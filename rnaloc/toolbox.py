...
# Function to read annotations from ImJoys' annotor plugin
...
def calc_expression_density_plot(fq_dict,img_size,outline_int = 'max',flag_plot = False,name_save=None,log_msg_callback=None,log_prog_callback=None):
    ...
    iCell = 1
    Ncells = len(fq_dict['cells'])
    
    log_message(f'Loop over {Ncells} cells',callback_fun = log_msg_callback)
    log_progress(iCell, Ncells, callback_fun = log_prog_callback)
    ...
    for key, value in fq_dict['cells'].items():
        ...
        [rr_cont, cc_cont] = polygon_perimeter(cell_pos[1,:], cell_pos[0,:], shape=img_outline.shape, clip=True)
        img_outline[rr_cont, cc_cont] = 1

        # Generate coordinates of pixels within polygon.
        rr, cc = polygon(cell_pos[1,:], cell_pos[0,:])
